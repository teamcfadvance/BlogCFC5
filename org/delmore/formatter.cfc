...
<cfcomponent output="false">
	...
	<cffunction name="getHTMLOutput" access="public" hint="This function accepts a block of code and formats it into syntax highlighted HTML." output="false">
		...
		<cfscript>
			...
			if (getConfig().getShowToolbar()) {
				variables.buffer.append("<iframe name='print_frame_" & arguments.codesig & "' style='display:inline;height:0px;width:0px;' frameborder=0></iframe>");
				
				// begin toolbar span
				variables.buffer.append("<div style='" & getStyle("TOOLBAR") & "'><span style='width:3em'>&nbsp;&nbsp;&nbsp</span>");
				...
				variables.buffer.append("<a href='javascript:show_about_" & arguments.codesig & "()' style='width:4em;" & getStyle("TOOLBARLINK") & "'>about</a>");
				
				// end toolbar
				variables.buffer.append("</div>");
				
				variables.buffer.append("<span id='about_" & arguments.codesig & "' style='display:none;border:" & getStyle("TEXT") & "'><span style='" & getStyle("LINENUMBER") & "'>&nbsp;</span>&nbsp;ColdFISH is developed by Jason Delmore.  Source code and license information available at <a href='http://coldfish.riaforge.org/'>coldfish.riaforge.org</a><br/></span>");
				
				variables.buffer.append("<span id='htmlencoded_plain_" & arguments.codesig & "' style='display:none;" & getStyle("TEXT") & "'>" & htmlcodeformat(arguments.code) & "</span>");				
			}
    ...
    <cffunction name="keywordsearch_old" access="private" hint="This function searches for keywords." output="false">
		...
		<cfset var keywordtypes = ""/>
		<cfset var keyword = ""/>
		<cfset var parser = ""/>
		<!--- TODO : Need to revisit performance here... a lot of time is spent on keyword searching... should store as a struct rather than list --->
		<cfloop collection="#getConfig().getKeywordmap()#" item="parser">
			<cfif evaluate("variables.is#parser#")><!--- check if we have defined keywords for this parser --->
				<cfset keywordmap = structfind(getConfig().getKeywordmap(),parser)/>
				<cfloop from="1" to="#ArrayLen(keywordmap.keywordlist)#" index="keywordtypes">
	                <cfloop list="#keywordmap.keywordlist[keywordtypes]#" index="keyword">
	                    <cfif regionMatches(thisLine, 1, i, keyword & ' ', 0, keyword.length()+1) or regionMatches(thisLine, 1, i, keyword & '(', 0, keyword.length()+1)>
	                        <cfset variables.buffer.append("<span id='keyword_DEBUG' style='" & getStyle(keywordmap.typename[keywordtypes]) & "'>" & keyword & "</span>")/>
    ...
    <cffunction name="startHighlight" access="private" hint="" output="false">
		...
		<cfset variables.buffer.append("<span id='" & arguments.element & "_DEBUG' style='" & getStyle(arguments.element) & "'>")/>
    </cffunction>
